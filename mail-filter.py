...
# - Only show mails matching filter.
# - Delete mails matching filter.
# - Pep8 safe.
...
parser.add_argument("-p", "--password", type=str, default=None,
        help="POP mail user password.")

parser.add_argument("-m", "--maxlines", type=str, default=10,
        help="Max number of lines to show for each mail. Default: 10.")

parser.add_argument("-f", "--filter", type=str, default="",
        help="Pattern as mail filter. Many filters must be separated by \
                colon.")

parser.add_argument("--delete", action="store_true", default=False,
        help="Mark for delete all matching mails.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        POPPASS = getpass.getpass("Password for %s@%s:" % (args.user,
            args.host)) if not args.password else args.password
        ...
        while msgnum <= stat[0] and not bye:
            ...
            if args.filter.strip():
                if True in [f.lower() in l.lower() for f in
                        args.filter.split(",") for l in lines]:
                    match = True
            else:
                match = True
        ...
        # Summary
        print "Deleting %d message(s) in mailbox %s@%s" % (count_del,
                args.user, args.host)
